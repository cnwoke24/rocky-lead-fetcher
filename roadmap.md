# Mike's Motor Zone Retention Demo

- [x] Build centralized retention demo data and mock services
- [x] Rebuild `/auto-demo` shell, overview, customers, and outreach flow
- [x] Add upload, campaigns, calls, integrations, analytics, and settings views
- [x] Add semantic dashboard colors and responsive behavior
- [x] Verify desktop/mobile interactions and console output
- [x] Connect Retell agent `agent_207fae2372f9309f151c8bb69b` with from-number +1(472)226-1802 to the auto-demo dashboard call flow
- [x] Replace the Retell credential, verify it can access the configured agent/number, and surface provider errors clearly- [x] Receive call results via webhook and surface them in the auto-demo dashboard

- [ ] Fix post-call results (visit confirmed detection), new outcome fields, follow-up rules, confirmation celebration
