...
serve(async (req) => {
  ...
  try {
    console.log('[GET-CALLS] Function invoked');
    ...
    if (authError || !user) {
      console.error('[GET-CALLS] Auth error:', authError);
      throw new Error('Unauthorized');
    }

    console.log('[GET-CALLS] User authenticated:', user.email);
    ...
    if (profileError || !profile) {
      console.error('[GET-CALLS] Profile error:', profileError);
      throw new Error('Profile not found');
    }

    if (!profile.clinic_id) {
      console.log('[GET-CALLS] No clinic assigned to user');
      return new Response(
    ...
    console.log('[GET-CALLS] Fetching calls for clinic:', profile.clinic_id);
    ...
    console.error('[GET-CALLS] Error:', errorMessage);
    return new Response(
