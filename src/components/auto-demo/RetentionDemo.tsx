...
function Sidebar({ view, navigate }: { view: DemoView; navigate: (view: DemoView) => void }) {
  return <><div className="flex h-[76px] items-center justify-center"><div className="grid size-10 place-items-center rounded-lg bg-primary"><img src={rockyLogo} alt="Rocky Voice AI" className="size-7 object-contain" /></div></div><nav className="flex flex-1 flex-col items-center gap-3 px-3 pt-4">{navItems.map((item) => <Button key={item.id} title={item.label} aria-label={item.label} variant="ghost" size="icon" onClick={() => navigate(item.id)} className={cn("size-11 text-demo-navy-muted hover:bg-demo-navy-accent hover:text-demo-navy-foreground", view === item.id && "bg-demo-navy-accent text-primary hover:bg-demo-navy-accent hover:text-primary")}><item.icon className="size-[18px]" /></Button>)}</nav><div className="pb-5"><div title="Mike’s Motor Zone" className="grid size-10 place-items-center rounded-lg bg-retention text-sm font-bold text-retention-foreground">M</div></div></>;
}
