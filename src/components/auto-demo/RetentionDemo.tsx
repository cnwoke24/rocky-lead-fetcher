...
import {
  Activity as ActivityIcon, BarChart3, Bot, CalendarCheck2, Check, CheckCircle2, ChevronLeft, ChevronRight,
  CircleDollarSign, Database, FileSpreadsheet, History, LayoutDashboard,
  Mail, Menu, PanelLeftOpen, PartyPopper, PhoneCall, Repeat2, Search, Settings, ShieldCheck, Sparkles, Target, Upload, UserRound, Users, X,
} from "lucide-react";
...
import mikesLogo from "@/assets/mikes-motor-zone-logo.png.asset.json";
import {
...
export function RetentionDemo() {
  ...
  return (
    <div className="auto-demo-shell min-h-screen bg-app text-foreground lg:flex">
      <aside className={cn("fixed inset-y-0 left-0 z-40 hidden flex-col bg-demo-navy text-demo-navy-foreground shadow-xl transition-[width] duration-300 ease-out lg:flex", sidebarExpanded ? "w-64" : "w-[72px]")}>
        <Sidebar view={view} navigate={navigate} expanded={sidebarExpanded} onToggle={() => setSidebarExpanded((current) => !current)} />
      </aside>
      {mobileNav && <div className="fixed inset-0 z-50 lg:hidden"><Button aria-label="Close navigation" variant="ghost" className="absolute inset-0 h-full w-full rounded-none bg-demo-navy/60" onClick={() => setMobileNav(false)} /><aside className="relative h-full w-[min(82vw,280px)] bg-demo-navy text-demo-navy-foreground shadow-2xl animate-fade-in"><Sidebar view={view} navigate={navigate} expanded mobile onToggle={() => setMobileNav(false)} /></aside></div>}

      <div className={cn("min-w-0 flex-1 transition-[margin] duration-300 ease-out", sidebarExpanded ? "lg:ml-64" : "lg:ml-[72px]")}>
...
const pageSubtitle: Record<DemoView, string> = { overview: "", customers: "Track every customer’s visit stage and next best action.", campaigns: "Monitor Rocky’s retention outreach programs.", calls: "Review call outcomes, confirmed visits, follow-ups, and transcripts.", analytics: "Measure conversion, bookings, recovery, and revenue.", settings: "Review preferences for this demonstration account." };

function Sidebar({ view, navigate, expanded, mobile = false, onToggle }: { view: DemoView; navigate: (view: DemoView) => void; expanded: boolean; mobile?: boolean; onToggle: () => void }) {
  return <TooltipProvider delayDuration={150}><div className={cn("flex h-[104px] items-center border-b border-demo-navy-accent px-3", expanded ? "justify-between gap-2" : "flex-col justify-center gap-1.5")}><div className={cn("grid shrink-0 place-items-center overflow-hidden rounded-md bg-card transition-all duration-300", expanded ? "h-[72px] w-[184px] p-1" : "size-11 cursor-pointer hover:ring-2 hover:ring-retention/60")} onClick={!expanded && !mobile ? onToggle : undefined} role={!expanded && !mobile ? "button" : undefined} aria-label={!expanded && !mobile ? "Expand navigation" : undefined}>
    {expanded ? <img src={mikesLogo.url} alt="Mike's Motor Zone Sales and Service" className="h-full w-full object-contain" /> : <span className="text-lg font-black text-retention">M</span>}
  </div>{!expanded && !mobile && <Button aria-label="Expand navigation" variant="ghost" size="icon" onClick={onToggle} className="size-6 text-demo-navy-muted hover:bg-demo-navy-accent hover:text-demo-navy-foreground"><PanelLeftOpen className="size-3.5" /></Button>}{expanded && <Button aria-label={mobile ? "Close navigation" : "Collapse navigation"} variant="ghost" size="icon" onClick={onToggle} className="size-8 shrink-0 text-demo-navy-muted hover:bg-demo-navy-accent hover:text-demo-navy-foreground">{mobile ? <X className="size-4" /> : <ChevronLeft className="size-4" />}</Button>}</div><nav className={cn("flex flex-1 flex-col gap-2 pt-5", expanded ? "px-3" : "items-center px-3")}>{navItems.map((item) => <UiTooltip key={item.id}><TooltipTrigger asChild><Button aria-label={item.label} variant="ghost" onClick={() => navigate(item.id)} className={cn("relative h-11 overflow-hidden text-demo-navy-muted transition-all duration-200 hover:bg-demo-navy-accent hover:text-demo-navy-foreground", expanded ? "w-full justify-start gap-3 px-3" : "w-11 justify-center px-0", view === item.id && "bg-demo-navy-accent text-demo-navy-foreground shadow-[inset_3px_0_0_hsl(var(--primary))] hover:bg-demo-navy-accent")}><item.icon className={cn("size-[18px] shrink-0 transition-colors", view === item.id && "text-primary")} />{expanded && <span className="truncate text-sm font-medium">{item.label}</span>}</Button></TooltipTrigger>{!expanded && <TooltipContent side="right">{item.label}</TooltipContent>}</UiTooltip>)}</nav>{!mobile && <div className={cn("border-t border-demo-navy-accent p-3", expanded ? "space-y-3" : "flex flex-col items-center gap-3")}><div className={cn("flex items-center gap-3", !expanded && "justify-center")}><div className="grid size-9 shrink-0 place-items-center rounded-md bg-retention text-sm font-bold text-retention-foreground">M</div>{expanded && <div className="min-w-0"><p className="truncate text-xs font-semibold">Mike’s Motor Zone</p><p className="text-[10px] text-demo-navy-muted">Sales & Service</p></div>}</div><Button aria-label="Expand navigation" variant="ghost" size="icon" onClick={onToggle} className={cn("text-demo-navy-muted hover:bg-demo-navy-accent hover:text-demo-navy-foreground", expanded && "hidden")}><PanelLeftOpen className="size-4" /></Button></div>}</TooltipProvider>;
}
...
function Overview({ customers, activities, calls, onCustomer, onCall, onNavigate, onSendEmail, onDemoCall, demoCalling }: { customers: Customer[]; activities: Activity[]; calls: CallRecord[]; onCustomer: (id: string) => void; onCall: (call: CallRecord) => void; onNavigate: (view: DemoView) => void; onSendEmail: () => void; onDemoCall: () => void; demoCalling: boolean }) {
  return <div className="space-y-8"><JourneyPanel /><div className="grid gap-6 xl:grid-cols-12"><div className="xl:col-span-7"><AutomationQueue onDemoCall={onDemoCall} demoCalling={demoCalling} /></div><div className="xl:col-span-5"><PerformanceOverview /></div></div><div className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]"><Card data-dashboard-card><CardHeader className="flex-row items-center justify-between"><div><CardTitle>Customers At Risk</CardTitle><CardDescription>Highest-value retention opportunities</CardDescription></div><Button variant="outline" size="sm" onClick={() => onNavigate("customers")}>View all</Button></CardHeader><CardContent className="overflow-x-auto p-0"><CustomerTable customers={customers.slice(0, 4)} onCustomer={onCustomer} compact /></CardContent></Card><ActivityFeed activities={activities} /></div><div className="grid gap-6 xl:grid-cols-2"><LastCall call={calls[0]} onTranscript={() => onCall(calls[0])} /><EmailPreview onSend={onSendEmail} /></div></div>;
}
...
function AutomationQueue({ onDemoCall, demoCalling }: { onDemoCall: () => void; demoCalling: boolean }) {
  const [paused, setPaused] = useState<string[]>([]);
  return <Card data-dashboard-card className="h-full"><CardHeader className="flex-row items-start justify-between"><div><CardTitle className="text-lg">Automations up next</CardTitle><CardDescription>The next steps Rocky will take for your customers.</CardDescription></div><Badge className="bg-primary/10 text-primary hover:bg-primary/10">{automations.length} queued</Badge></CardHeader><CardContent className="divide-y p-0 px-5">{automations.map((item) => { const Icon = item.icon; const isPaused = paused.includes(item.id); const isDemo = "demo" in item && item.demo; return <div key={item.id} className="grid gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="flex min-w-0 items-start gap-3"><div className={cn("grid size-10 shrink-0 place-items-center rounded-lg border bg-card text-primary", isDemo && "border-retention/30 bg-retention-soft text-retention-soft-foreground")}><Icon className="size-4" /></div><div className="min-w-0"><p className="text-sm font-semibold">{item.title}</p><p className="mt-0.5 text-xs text-muted-foreground">{item.detail}</p></div></div><div className="flex items-center justify-between gap-4 pl-[52px] sm:pl-0"><div><p className="text-xs font-semibold">{item.time}</p><p className="text-[11px] text-muted-foreground">{item.type}</p></div>{isDemo
...
function CustomerDetail({ customer, update, chooseScenario, completeVisit, startOutreach, sendDemoEmail, lastCall, onTranscript, onSaveRecord, savingRecord, onDemoCall, demoCalling, onMarkVisit }: { customer: Customer | null; update: (patch: Partial<Customer>) => void; chooseScenario: (visits: number) => void; completeVisit: () => void; startOutreach: () => void; sendDemoEmail: () => void; lastCall: CallRecord; onTranscript: () => void; onSaveRecord: (customer: Customer) => void; savingRecord: boolean; onDemoCall: (slug: string) => void; demoCalling: boolean; onMarkVisit: (customer: Customer) => void }) {
  ...
  return <div><SheetHeader className={cn("border-b p-6 pr-12", confirmed && !missed && "bg-success/10")}><div className="flex items-start gap-3"><div className={cn("grid size-11 place-items-center rounded-md bg-primary text-sm font-bold text-primary-foreground", confirmed && !missed && "bg-success text-success-foreground")}>{customer.firstName[0]}{customer.lastName[0]}</div><div><SheetTitle className="text-xl">{fullName(customer)}</SheetTitle><SheetDescription>{customer.vehicle} · {customer.visitStage}</SheetDescription></div>{confirmed && <Badge className={cn("ml-auto", statusClass(missed ? "At Risk" : "Visit Confirmed"))}>{missed ? "Missed visit" : "Visit Confirmed"}</Badge>}</div></SheetHeader><div className="space-y-6 p-6">
    {confirmed && (missed
      ? <div className="rounded-md border border-warning/40 bg-warning/15 p-4"><div className="flex items-center gap-2 font-semibold text-warning-foreground"><History className="size-4" /> Missed visit — follow up</div><p className="mt-1 text-sm text-muted-foreground">{customer.firstName} said they would come in {confirmed.day}, but no completed visit has been recorded. Time to follow up.</p><div className="mt-3 flex flex-wrap gap-2">{customerSlug && <Button size="sm" disabled={demoCalling} className="bg-retention text-retention-foreground hover:bg-retention/90" onClick={() => onDemoCall(customerSlug)}><PhoneCall /> {demoCalling ? "Dialing…" : "Call to follow up"}</Button>}<Button size="sm" variant="outline" onClick={() => onMarkVisit(customer)}><CheckCircle2 /> They came in — mark visit completed</Button></div></div>
