    ...
    <div className="min-h-screen bg-app text-foreground lg:flex">
      ...
      <div className="min-w-0 flex-1 lg:ml-64">
        ...
        <main className="mx-auto max-w-[1600px] space-y-6 p-4 lg:p-7">
          ...
          {view === "overview" && <Overview customers={customers} activities={activities} calls={calls} onCustomer={setSelectedId} onCall={setSelectedCall} onNavigate={navigate} />}
...
function ImportPreview({ rows, analyzed, onAnalyze }: { rows: ImportedRow[]; analyzed: boolean; onAnalyze: () => void }) { const columns = analyzed ? ["first_name", "last_name", "vehicle_make", "lifetime_visits", "visit_stage", "risk_status", "next_best_action"] : importFields.slice(0, 7); return <div className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold">{rows.length} customers imported successfully.</p><p className="text-sm text-muted-foreground">{analyzed ? "Rocky assigned lifecycle stages and next actions." : "Review the first records, then analyze the list."}</p></div>{!analyzed && <Button onClick={onAnalyze}><Sparkles /> Analyze Customers</Button>}</div><div className="rounded-md border"><Table><TableHeader><TableRow>{columns.map((column) => <TableHead key={column} className="whitespace-nowrap text-xs">{column.replaceAll("_", " ")}</TableHead>)}</TableRow></TableHeader><TableBody>{rows.slice(0, 6).map((row, index) => <TableRow key={index}>{columns.map((column) => <TableCell key={column} className="whitespace-nowrap text-xs">{String(row[column] ?? "—")}</TableCell>)}</TableRow>)}</TableBody></Table></div><p className="text-xs text-muted-foreground">Previewing 6 of {rows.length} imported customers.</p></div>; }
