...
function AutomationQueue({ onDemoCall, demoCalling }: { onDemoCall: () => void; demoCalling: boolean }) {
  ...
  return <Card data-dashboard-card className="h-full"><CardHeader className="flex-row items-start justify-between"><div><CardTitle className="text-lg">Automations up next</CardTitle><CardDescription>The next steps Rocky will take for your customers.</CardDescription></div><Badge className="bg-primary/10 text-primary hover:bg-primary/10">{automations.length} queued</Badge></CardHeader><CardContent className="divide-y p-0 px-5">{automations.map((item) => { const Icon = item.icon; const isPaused = paused.includes(item.id); return <div key={item.id} className={cn("grid gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center", "demo" in item && item.demo && "rounded-lg")}><div className="flex min-w-0 items-start gap-3"><div className={cn("grid size-10 shrink-0 place-items-center rounded-lg border bg-card text-primary", "demo" in item && item.demo && "border-retention/30 bg-retention-soft text-retention-soft-foreground")}><Icon className="size-4" /></div><div className="min-w-0"><p className="text-sm font-semibold">{item.title}</p><p className="mt-0.5 text-xs text-muted-foreground">{item.detail}</p></div></div><div className="flex items-center justify-between gap-4 pl-[52px] sm:pl-0"><div><p className="text-xs font-semibold">{item.time}</p><p className="text-[11px] text-muted-foreground">{item.type}</p></div>{"demo" in item && item.demo
