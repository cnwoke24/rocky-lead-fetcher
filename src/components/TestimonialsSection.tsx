...
function TestimonialCard({ t }: { t: Testimonial }) {
  return (
    <figure className="rounded-2xl border border-slate-200 bg-white/70 p-6 shadow-sm backdrop-blur">
      ...
      <figcaption className="mt-6 flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="truncate text-xs text-slate-500 hover:text-slate-700"
          >
  ...
  const testimonials: Testimonial[] = [
    {
      ...
      tone: "blue",
      quote:
        "Our calls stopped piling up and the team isn't tied to the phone anymore. Patients get helped right away, even when the front desk is busy.",
      useCase:
        "Inbound call handling + intake so they didn't need to overstaff the front desk to avoid missed calls.",
    },
    {
      ...
      tone: "purple",
      quote:
        "We can focus on sessions instead of interrupting them to answer calls. Booking feels smooth and we're not losing appointments when things get hectic.",
      useCase:
        "AI receptionist that answers questions and books appointments so the owner can grow without hiring a full-time receptionist.",
    },
    {
      ...
      tone: "slate",
      quote:
        "The outreach runs consistently every week. Members who fell off get a clear reason to come back, and the team only steps in when someone's interested.",
      useCase:
        "Outbound calling to inactive members with a 'return special' to improve retention and reactivations.",
    },
  ];
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Testimonials are short, summarized notes based on client feedback.
          </p>
        </div>

        {/* Logo strip */}
        <div className="mx-auto mt-10 flex max-w-4xl flex-wrap items-center justify-center gap-6 rounded-2xl border border-slate-200 bg-white/60 px-6 py-5 shadow-sm backdrop-blur">
          {testimonials.map((t) => (
            <a
              key={t.name}
              href={t.website}
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-3 rounded-xl px-3 py-2 hover:bg-white"
            >
