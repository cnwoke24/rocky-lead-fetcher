...
const Check = ({ tone }: { tone: Tone }) => (
  ...
  >
    <svg
      viewBox="0 0 20 20"
      fill="currentColor"
      className={"h-4 w-4 " + TONE[tone].accentText}
      aria-hidden="true"
    >
      <path
        fillRule="evenodd"
        d="M16.704 5.29a1 1 0 010 1.414l-7.07 7.07a1 1 0 01-1.414 0l-3.535-3.535a1 1 0 111.414-1.414l2.828 2.828 6.364-6.364a1 1 0 011.414 0z"
        clipRule="evenodd"
      />
    </svg>
  </span>
  ...
  return (
    <div
      className={
        ...
        TONE[tone].border +
        (plan.featured ? " ring-2 " + TONE[tone].ring + " shadow-md" : "")
      }
    >
      {plan.badge ? (
      ...
      <div className="mt-6">
        ...
        {plan.note ? (
          <div
            className={
              "mt-5 rounded-2xl border p-4 text-xs text-slate-700 " +
              TONE[tone].border +
              " bg-white/60"
            }
          >
      ...
      <button
        className={
          "mt-8 w-full rounded-2xl px-5 py-3 text-sm font-semibold text-white shadow-sm transition active:scale-[0.99] bg-gradient-to-r " +
          TONE[tone].gradient
        }
      >
        {plan.cta}
  ...
  const plans: Plan[] = [
    {
      ...
      features: [
        ...
        "Caller information capture + confirmation",
        "Call logging + summary",
        "Email confirmations (instant follow-ups)",
        "Intelligent routing for new vs. returning callers",
        "Email support",
      ],
      cta: "Get Started",
      accent: "blue",
    },
    {
      ...
      features: [
        ...
        "Webhook/automation routing for booking + updates + cancellations",
        "Calendar + CRM connection (event creation + contact logging)",
      ...
      note:
        "Best for teams that want full automation: booking workflows, CRM/calendar logging, reminders, and transfer routing.",
      cta: "Choose Pro",
      accent: "purple",
      featured: true,
    },
    {
      badge: { label: "💰 Lower Monthly", tone: "slate" },
      name: "Saver",
      setupFee: "$2,000 setup fee",
      price: "$499",
      priceSuffix: "/mo",
      features: [
        "Client dashboard login for call analytics",
        "Call transfer options",
        "Slack channel support",
        "Everything in the Launch & Pro plans",
      ],
      note:
        "Ideal if you want a lower monthly rate and can pay the one-time setup fee upfront.",
      cta: "Choose Saver",
      accent: "slate",
    },
  ];
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Colors and gradients match the Rocky AI homepage style. Add-ons can
            be quoted if you need custom workflows.
          </p>
        </div>
